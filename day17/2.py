...
# Except no - part 2 needs us to count hits - winner, winner chicken dinner...

def tosser(target):
    print(target)
    best = { 'ix': 0, 'iy':0, 'maxy': 0 }
    for ix in range(target.xmax+1): # Can only range from 0 (if we are above it) to target(xmax) - if we took a single horiz step.
        for iy in range(target.ymin-1,200): # 200 is arbitrary upper-bound, start of range is single down-step...
